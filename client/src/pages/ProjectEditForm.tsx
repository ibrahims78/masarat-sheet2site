  ...
  if ((record as any)?.error || error) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 p-4">
      <Card className="p-8 max-w-sm w-full text-center space-y-3">
        ...
        <h2 className="font-bold text-slate-800">{(record as any)?.error || "الرابط غير صالح"}</h2>
      </Card>
    </div>
  );
  ...
  if (saved) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 p-4">
      <Card className="p-8 max-w-sm w-full text-center space-y-4">
        ...
        <h2 className="text-xl font-bold text-slate-800">تم حفظ التعديلات بنجاح!</h2>
      </Card>
    </div>
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 py-8 px-4" dir="rtl">
      <div className="max-w-xl mx-auto space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{project?.formTitle || "تعديل البيانات"}</h1>
          <p className="text-sm text-muted-foreground mt-1">يمكنك تعديل بياناتك أدناه</p>
        </div>
          ...
          {Object.entries(grouped).map(([step, stepFields]) => (
            <Card key={step} className="p-5">
              <h3 className="text-sm font-bold text-slate-700 dark:text-slate-200 mb-4 pb-2 border-b border-slate-100 dark:border-slate-700">
                {Array.isArray(project?.steps) ? (project.steps[Number(step) - 1] || `الخطوة ${step}`) : `الخطوة ${step}`}
              </h3>
