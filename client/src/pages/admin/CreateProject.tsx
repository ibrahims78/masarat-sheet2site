...
const WIZARD_STEPS = ["رفع الملف", "تحرير الحقول", "إعدادات المشروع", "إنشاء"];
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold">إنشاء مشروع جديد</h1>
        </div>
            ...
            >
              ...
              {uploading ? (
                ...
                <div className="space-y-3">
                  ...
                  <h3 className="font-semibold text-slate-700 dark:text-slate-200">ارفع ملف Excel</h3>
                  <p className="text-sm text-muted-foreground">اسحب ملف .xlsx أو انقر للاختيار</p>
                  <p className="text-xs text-muted-foreground">سيتم استخراج الأعمدة تلقائياً كحقول للمشروع</p>
                </div>
              )}
            </Card>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="font-semibold text-slate-800 dark:text-slate-100">تحرير الحقول</h2>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {manualMode ? "أضف الحقول التي ستظهر في النموذج" : `تم العثور على ${columns.length} حقل — اختر وعدّل ما تريد`}
                </p>
              </div>
              {!manualMode && (
            ...
            <div className="space-y-2 max-h-[55vh] overflow-y-auto pr-1">
              {columns.length === 0 && (
                <p className="text-center text-sm text-muted-foreground py-8">لا يوجد حقول. أضف حقلاً للبدء.</p>
              )}
              {columns.map((col, idx) => (
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-4 gap-2">
                      ...
                      <div className="sm:col-span-2 space-y-1">
                        <p className="text-[10px] text-muted-foreground">الاسم المعروض</p>
                      ...
                      <div className="space-y-1">
                        <p className="text-[10px] text-muted-foreground">المفتاح</p>
        ...
        {step === 2 && (
          <Card className="p-6 space-y-5">
            <h2 className="font-semibold text-slate-800 dark:text-slate-100">إعدادات المشروع</h2>
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">وصف المشروع (اختياري)</Label>
              <Textarea value={description} onChange={e => setDescription(e.target.value)} rows={2} data-testid="input-description" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-xs">عنوان النموذج العام</Label>
                <Input value={formTitle} onChange={e => setFormTitle(e.target.value)}
                  placeholder="نموذج التسجيل" data-testid="input-form-title" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">العنوان الفرعي (اختياري)</Label>
                <Input value={formSubtitle} onChange={e => setFormSubtitle(e.target.value)}
                  placeholder="برجاء تعبئة البيانات بدقة" data-testid="input-form-subtitle" />
              </div>
            </div>

            <div className="space-y-1.5">
              <Label className="text-xs">أسماء الخطوات (كل خطوة في سطر — آخر خطوة للمراجعة)</Label>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <Card className="p-6 space-y-4">
              <h2 className="font-semibold text-slate-800 dark:text-slate-100">مراجعة وإنشاء</h2>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  <p className="text-xs text-muted-foreground">رمز الدعوة</p>
                  <p className="font-mono font-semibold">{invitationCode || `${projectName.replace(/\s+/g, "-").toUpperCase()}-2026`}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">عنوان النموذج</p>
                  <p className="font-semibold">{formTitle || projectName}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">عدد الحقول</p>
                  <p className="font-semibold">{selectedCount} حقل</p>
                </div>
              </div>

              <div>
                <p className="text-xs text-muted-foreground mb-2">الخطوات</p>
              ...
              <div>
                <p className="text-xs text-muted-foreground mb-2">الحقول المحددة ({selectedCount})</p>
            ...
            {createMut.isError && (
              <div className="p-3 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-700 text-sm" data-testid="text-create-error">
                حدث خطأ أثناء الإنشاء. حاول مرة أخرى.
              </div>
            )}
