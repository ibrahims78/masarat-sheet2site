                      ...
                      <tr key={record.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors group" data-testid={`row-record-${record.id}`}>
                        ...
                        {colFields.map(f => {
                          ...
                          if (f.fieldType === "file") {
                            ...
                            const fileUrls: string[] = Array.isArray(val)
                              ? (val as any[]).map(String).filter(Boolean)
                              : (val ? [String(val)] : []);
                            return (
                                    ...
                                    {fileUrls.map((url, fi) => (
                                      <div key={fi} className="flex items-center gap-1.5">
                                        <a
                                          ...
                                          data-testid={`link-file-${copyKey}-${fi}`}
                                        >
                                          {fileUrls.length === 1
                                            ? (isAr ? "عرض الملف" : "View File")
                                            : (isAr ? `ملف ${fi + 1}` : `File ${fi + 1}`)}
                                        </a>
                                        {fi === 0 && <span title={syncTitle} className="shrink-0 text-xs">{syncIcon}</span>}
                                      </div>
                                    ))}
                                  </div>
                                ) : (
