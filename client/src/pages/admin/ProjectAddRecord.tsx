  ...
  return (
    <Layout projectId={id}>
      <form onSubmit={handleSubmit(d => addMut.mutate(d))} className="max-w-3xl space-y-5">
        ...
        {addMut.isError && (
          <div className="p-3 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 text-sm">حدث خطأ أثناء الإضافة</div>
        )}
        ...
        {fields.length === 0 && (
          <Card className="p-10 text-center text-muted-foreground text-sm">لا يوجد حقول. أضف حقولاً من إعدادات المشروع أولاً.</Card>
        )}
      </form>
    </Layout>
  );
