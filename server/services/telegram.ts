...
export async function sendTelegramNotification(employeeData: {
  firstName: string;
  familyName: string;
  nationalId: string;
  jobTitle?: string | null;
  workGovernorate?: string | null;
}) {
  try {
    const [settings] = await db.select().from(systemSettings).where(eq(systemSettings.id, "singleton"));

    if (!settings?.telegramBotTokenEnc) {
      console.log("[Telegram] ⚠️ Bot Token غير محفوظ — الإشعار ملغى");
      return;
    }
    if (!settings?.telegramChatId) {
      console.log("[Telegram] ⚠️ Chat ID غير محفوظ — الإشعار ملغى");
      return;
    }

    const token = decrypt(settings.telegramBotTokenEnc);
    if (!token) {
      console.log("[Telegram] ⚠️ فشل فك تشفير Bot Token");
      return;
    }

    const message =
      `🏥 *تسجيل جديد — نظام الكوادر الصحية*\n\n` +
      `👤 الاسم: ${employeeData.firstName} ${employeeData.familyName}\n` +
      `🆔 الرقم الوطني: \`${employeeData.nationalId}\`\n` +
      `💼 المسمى الوظيفي: ${employeeData.jobTitle || "—"}\n` +
      `📍 المحافظة: ${employeeData.workGovernorate || "—"}\n` +
      `🕒 الوقت: ${new Date().toLocaleString("ar-SY", { timeZone: "Asia/Damascus" })}`;
    ...
    if (result.ok) {
      console.log(`[Telegram] ✅ إشعار أُرسل لـ ${employeeData.firstName} ${employeeData.familyName}`);
    } else {
      console.error(`[Telegram] ❌ فشل الإرسال: ${result.description}`);
    }
  } catch (err) {
    console.error("[Telegram] ❌ خطأ:", err);
  }
}
